...
# Locate 'TABLE OF CONTENTS' heading and actual Chapter 1 body heading
toc_heading_p = None
ch1_body_p = None

for i, p in enumerate(doc.paragraphs):
    txt = p.text.strip()
    if txt == "TABLE OF CONTENTS" and toc_heading_p is None:
        toc_heading_p = p
    elif txt.startswith("Chapter 1") and "Executive Summary" in txt and "....." not in txt and ch1_body_p is None:
        ch1_body_p = p

print("Found headings:", toc_heading_p is not None, ch1_body_p is not None)

# Delete ALL paragraphs that sit strictly between 'TABLE OF CONTENTS' and 'Chapter 1 - Executive Summary'
p_to_remove = []
collect = False
...
print(f"Removing {len(p_to_remove)} old TOC paragraphs...")
for p in p_to_remove:
    p._element.getparent().remove(p._element)

# Reload document cleanly
doc = docx.Document(file_path)
...
# Insert BEFORE ch1_body_p in forward order
for num, title in toc_entries:
    ...
    new_p = ch1_body_p.insert_paragraph_before(full_toc_line)
...
doc.save(file_path)
print("SUCCESSFULLY REBUILT CLEAN TOC!")
