...
# 1. Find TABLE OF CONTENTS heading and Chapter 1 heading
toc_idx = -1
ch1_idx = -1

for i, p in enumerate(doc.paragraphs):
    txt = p.text.strip()
    if txt == "TABLE OF CONTENTS" and toc_idx == -1:
        toc_idx = i
    elif txt.startswith("Chapter 1") and "Executive Summary" in txt and not "...." in txt and ch1_idx == -1:
        ch1_idx = i

print(f"TOC index: {toc_idx}, Chapter 1 index: {ch1_idx}")

# Remove any paragraphs between TABLE OF CONTENTS and Chapter 1
if toc_idx != -1 and ch1_idx != -1 and ch1_idx > toc_idx + 1:
    to_remove = []
    for i in range(toc_idx + 1, ch1_idx):
...
# Find Chapter 1 heading index again
ch1_p = None
...
# Insert in normal forward order before ch1_p
...
doc.save(file_path)
print("PERFECT TOC CREATED IN FORWARD CHRONOLOGICAL ORDER!")
